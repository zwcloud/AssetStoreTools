...
namespace AssetStoreTools
{
    internal static class AssetStoreClient
    {
        ...
        private static string AssetStoreUrl
        {
            get
            {
                //Modified by zwcloud: embedded loader.htm from Unity 2019.4
                //string input = File.ReadAllText(EditorApplication.applicationContentsPath + "/Resources/loader.html");
                string input = @"<!DOCTYPE html>
<html><head><title>Asset Store</title><meta charset=""utf-8""><meta http-equiv=""Content-Type"" content=""text/html;charset=utf-8""><script type=""text/javascript""> (function(){ setTimeout(function(){location.href = 'https://kharma.unity3d.com';},100); })();</script></head><body oncontextmenu=""return false;"" style=""background-color: #333;""><div style=""position: absolute; left: 50%; top: 50%; margin-left: -100px; margin-top: -50px; width:200px; height:100px; font-size: 10px; color: #777; line-height: 12px; font-family: Helvetica, Verdana, Arial, sans-serif; text-align: center;""><img src=""data:image/gif;base64,""><br><br>Connecting to Asset Store</div></body></html>
";

                Match match = Regex.Match(input, "location.href.*?=.*?'(.*?//[^/']+)");
                if (match.Success)
                {
        ...
        private static Uri APIUri(string path, IDictionary<string, string> extraQuery)
        {
            ...
            dictionary.Add("toolversion", "V5.0.2");
        ...
        private static string ActiveSessionID
        {
            get
            {
                string text = AssetStoreClient.sActiveSessionIdBackwardsCompatibility;

                Assembly assembly = Assembly.Load("UnityEditor");
                Type assetStoreContexType = assembly.GetType("UnityEditor.AssetStoreContext");
                if (assetStoreContexType != null)
                {
                    MethodInfo method = assetStoreContexType.GetMethod("SessionGetString");
                    text = AssetStoreClient.SavedSessionID;
                    if (method != null)
                    {
            ...
            set
            {
                ...
                if (assetStoreContexType != null)
                {
                    MethodInfo method = assetStoreContexType.GetMethod("SessionSetString");
                    if (method != null)
                    {
        ...
        internal static void LoginWithCredentials(string username, string password, bool rememberMe, AssetStoreClient.DoneLoginCallback callback)
        {
            ...
            nameValueCollection.Add("toolversion", "V5.0.2");
        ...
        internal static void LoginWithAccessToken(AssetStoreClient.DoneLoginCallback callback)
        {
            if (AssetStoreClient.sLoginState == AssetStoreClient.LoginState.IN_PROGRESS)
            {
                DebugUtils.LogWarning("Tried to login with access token while already in progress of logging in");
                return;
            }
            AssetStoreClient.sLoginState = AssetStoreClient.LoginState.IN_PROGRESS;
            AssetStoreClient.sLoginErrorMessage = null;
            ...
            AssetStoreWebClient assetStoreWebClient = new AssetStoreWebClient();
            NameValueCollection nameValueCollection = new NameValueCollection();
            nameValueCollection.Add("user_access_token", UnityConnectSession.instance.GetAccessToken());
            nameValueCollection.Add("unityversion", Application.unityVersion);
            nameValueCollection.Add("toolversion", "V5.0.2");
            nameValueCollection.Add("license_hash", AssetStoreClient.GetLicenseHash());
            nameValueCollection.Add("hardware_hash", AssetStoreClient.GetHardwareHash());
        ...
        private static string UpdateLargeFilesUpload(AssetStoreClient.LargeFilePending assetUpload)
        {
            StreamReader streamReader = null;
            WebResponse webResponse = null;
            try
            {
                if (assetUpload == null || assetUpload.Request == null)
                {
                    return null;
                }
                byte[] buffer = assetUpload.Buffer;
                int num = 0;
                for (; ; )
                {
                    num++;
                    int num2 = assetUpload.RequestFileStream.Read(buffer, 0, buffer.Length);
                    if (num2 == 0)
                    {
                        break;
                    }
                    assetUpload.RequestStream.Write(buffer, 0, num2);
                    assetUpload.BytesSent += (long)num2;
                    if (num % 500 == 0)
                    {
                        goto Block_6;
                    }
                ...
                Block_6:
                    try
                    {
                        double num3 = (double)assetUpload.BytesSent;
                        double num4 = (double)assetUpload.BytesToSend;
                        double pctUp = num3 / num4 * 100.0;
                        if (assetUpload.RequestProgressCallback != null)
                        {
                            assetUpload.RequestProgressCallback(pctUp, 0.0);
                        }
                    }
                    catch (Exception ex)
                    {
                        DebugUtils.LogWarning("Progress update error " + ex.Message);
                    }
                }
                AssetStoreClient.s_PendingLargeFiles.Remove(assetUpload);
                AssetStoreClient.s_UploadingLargeFile = null;
                DebugUtils.Log("Finished Uploading: " + assetUpload.Id);
                webResponse = assetUpload.Request.GetResponse();
            ...
            {
                DebugUtils.LogError("UploadingLarge Files Exception:" + ex3.Source);
                if (streamReader != null)
                {
                    streamReader.Close();
                }
                AssetStoreResponse job2 = AssetStoreClient.parseAssetStoreResponse(null, null, ex3, (webResponse == null) ? null : webResponse.Headers);
                assetUpload.RequestDoneCallback(job2);
                assetUpload.Close();
                AssetStoreClient.s_PendingLargeFiles.Remove(assetUpload);
            }
            return null;
        ...
        public static void Update()
        {
            ...
            lock (obj)
            {
                ...
                IL_19F:
                    if (p.progressCallback != null && p.statsUpdated)
                    {
                        ...
                        double pctUp = (p.totalBytesToSend <= 0.0) ? 0.0 : (p.bytesSend / p.totalBytesToSend * 100.0);
                        double pctDown = (p.totalBytesToReceive <= 0.0) ? 0.0 : (p.bytesReceived / p.totalBytesToReceive * 100.0);
                        try
                        {
            ...
            AssetStoreClient.PrepareUpdateLargeFilesUpload();
        }

        private static void PrepareUpdateLargeFilesUpload()
        {
            ...
            if (AssetStoreClient.s_UploadingLargeFile != null && AssetStoreManager.sUploadThread == null)
            {
                AssetStoreManager.sUploadThread = new Thread(new ParameterizedThreadStart(AssetStoreClient.HandleLargeFilesUpload));
                AssetStoreManager.sUploadThread.Start(AssetStoreClient.s_UploadingLargeFile);
            }
        }

        internal static void HandleLargeFilesUpload(object o)
        {
            AssetStoreClient.LargeFilePending assetUpload = (AssetStoreClient.LargeFilePending)o;
            AssetStoreClient.UpdateLargeFilesUpload(assetUpload);
        }
        ...
        public const string TOOL_VERSION = "V5.0.2";
